...
raw_stats <- author_stats |> filter(.data$Data == "raw")
filtered_stats <- author_stats |> filter(.data$Data == "filtered")
```
...
# Data filtering
```{r, echo=FALSE}
max_authors_text <- if (network$n_too_many_vertices > 0) {
...
min_papers_text <- if (network$n_infrequent_vertices_removed > 0) {
...
# Network Description
```{r, echo=FALSE}
network_direction_text <- if (network$directed) {
...
- **Min**: `r round(min(get_centrality(graph, "harmonic")), digits = 5)` (The most distant node, or those in isolated groups with few reachable neighbors)
- **Max**: `r round(max(get_centrality(graph, "harmonic")), digits = 5)` (The most central node, can quickly reach others)
- **Mean**: `r round(mean(get_centrality(graph, "harmonic")), digits = 5)` (If the mean is high, the network is compact and well-connected. A low mean suggests the graph is fragmented, with many weakly connected groups.)
- **Median**: `r round(median(get_centrality(graph, "harmonic")), digits = 5)`
...
if (length(get_cutpoints(graph)) == 0) {
  ...
  cutpoint_table <- matrix(names(get_cutpoints(graph)), ncol = 4, byrow = TRUE)
}
```
`r cutpoint_text`
```{r, echo=FALSE, warning=FALSE}
...
plot_graph(graph, centrality = "betweenness")
```
...
```{r, echo=FALSE}
top_authors_table <- matrix(get_most_central_vertices(graph, n = 15, centrality = "betweenness"), ncol = 5, byrow = TRUE)
knitr::kable(top_authors_table, col.names = c("", "", "", "", ""), format = "markdown")
```
...
plot_graph(graph, centrality = "harmonic")
```
Each color represents a cluster of authors who are strongly connected within the network.
...
```{r, echo=FALSE}
top_authors_table <- matrix(get_most_central_vertices(graph, n = 15, centrality = "harmonic"), ncol = 5, byrow = TRUE)
knitr::kable(top_authors_table, col.names = c("", "", "", "", ""), format = "markdown")
```
...
plot_graph(graph, centrality = "none")
```
