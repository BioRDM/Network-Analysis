...
```
Input data: `r paste0(metadata$input_name, ".csv")`

Date range: `r config$data$from_year` to `r config$data$to_year`

`r metadata_text`

# Data filtering
```{r, echo=FALSE}
max_authors_text <- if (is.null(config$data$max_authors_per_paper)) {
  "No limit on the number of authors per paper was applied."
} else if (network$n_too_many_vertices > 0) {
  paste0(network$n_too_many_vertices, " papers were removed from the dataset due to having more than ", config$data$max_authors_per_paper, " authors.")
} else {
  paste0("All papers in the dataset had less than ", config$data$max_authors_per_paper, " authors and were included in the network analysis.")
}
min_papers_text <- if (is.null(config$data$min_papers_per_author)) {
  "No limit on the minimum number of papers per author was applied."
} else if (network$n_infrequent_vertices_removed > 0) {
  paste0(network$n_infrequent_vertices_removed, " authors were removed from the dataset because they appeared in less than ", config$data$min_papers_per_author, " papers.")
} else {
  paste0("All authors in the dataset appeared in at least ", config$data$min_papers_per_author, " papers and were included in the network analysis.")
}
```
...
```{r, fig.pos = 'H', fig.cap = "Visualisation of the Co-Authorship Network", echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, out.width='100%'}
graph <- set_centrality(graph, method = "degree")
plot(graph, vertex_color = vertex_color, vertex_size = "centrality", vertex_order = config$node_order, vertex_palette = config$node_palette)
```

```{r, fig.pos = 'H', fig.align='center', fig.cap = NULL, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, out.width='90%'}
plot_legend_only(graph, vertex_color = vertex_color, vertex_order = config$node_order, vertex_palette = config$node_palette)
```
...
print(plot_top_vertices(graph, vertex_color = vertex_color, n = 30, vertex_order = config$node_order, vertex_palette = config$node_palette))
```
...
- **Unreachable Pairs**: `r round((1 - get_reachability(graph))*100)`% of all possible pairs of nodes are unreachable in the network.
...
There are `r length(unique(igraph::V(graph$graph)$name[!is.na(igraph::V(graph$graph)$cutpoint)]))` cutpoint authors, whose names are listed below.
...
plot(graph, vertex_color = vertex_color, vertex_size = NULL, vertex_order = config$node_order, vertex_palette = config$node_palette)
```
...
```{r, fig.pos = 'H', fig.cap = "Visualisation of the Co-Authorship Network (without names)", echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, out.width='100%'}
graph <- set_centrality(graph, method = "degree")
plot(graph, vertex_color = vertex_color, vertex_size = "centrality", vertex_order = config$node_order, vertex_palette = config$node_palette, display_names=FALSE)
```
