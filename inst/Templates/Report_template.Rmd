...
```
Input data: `r paste0(config$.meta$input_name, ".csv")`

Date range: `r config$data$from_year` to `r config$data$to_year`

`r metadata_text`

Package Version: `r config$.meta$package_version`
...
plot(graph, vertex_color = "cutpoint", layout = config$plot$layout)
```
