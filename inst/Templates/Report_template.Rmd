...
```
The initial dataset contained `r length(unique(network$raw[[network$edge_id]]))` papers and `r length(unique(network$raw[[network$author_column_name]]))` authors from `r min(network$raw[[network$year_column]])` to `r max(network$raw[[network$year_column]])`.
