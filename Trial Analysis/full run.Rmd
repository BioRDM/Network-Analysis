...
data <- read.csv("E:\\Daniel\\NetworkAnalysis\\Pure Data\\Scopus 2023_24_v1.csv", stringsAsFactors = FALSE)
...
df_authors <- data %>%
  mutate(Authors = str_split(Author, "; ")) %>%
  select(Authors)

df_authors <- df_authors %>%
  filter(map_int(Authors, length) > 1)

# Create edges: pairwise combinations of co-authors
edges <- df_authors %>%
  mutate(pairs = map(Authors, ~combn(.x, 2, simplify = FALSE))) %>%
  unnest(pairs) %>%
...
write.graph(institution.graph, file = "author_network.net", format = "pajek")
