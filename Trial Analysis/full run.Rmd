...
data <- read.csv("Pure_23_24_Code.csv", stringsAsFactors = FALSE)
...
# Function to count authors based on the Source
count_authors <- function(author_column, source_column) {
  mapply(function(authors, source) {
    if (source == "PUBMed") {
...
# Apply the function to count authors
data$num_authors <- count_authors(data$Author, data$Source)

# Function to split authors conditionally
split_authors <- function(authors, source) {
  if (source == "PUBMed") {
    unlist(strsplit(authors, ","))
  } else {
    unlist(strsplit(authors, ";"))
  }
}

# Get all authors with conditional splitting
all_authors <- unlist(mapply(split_authors, data$Author, data$Source))

# Remove leading and trailing whitespace
all_authors <- trimws(all_authors)

# Count unique authors
num_unique_authors <- length(unique(all_authors))

# Print the result
cat("Total number of unique authors:", num_unique_authors, "\n")

# Calculate mean, median, min, max
avg_authors <- mean(data$num_authors)
median_authors <- median(data$num_authors)
min_authors <- min(data$num_authors)
max_authors <- max(data$num_authors)

# Print results
cat("Number of authors per paper:\n")
print(data[, c("Key", "num_authors")])
...
all_authors <- unlist(strsplit(data$Author, ";"))
...
#Switch the CSV file to iGraph file format
...
#Filter Datasets by papers which exclude papers with noe author or >= x number of publication here is 2
...
df_filtered <- data %>%
  ...
  mutate(Authors = str_split(Author, "; ")) %>% #how the authors names are splitted by;
  filter(map_int(Authors, length) > 0 & map_int(Authors, length) <= 50) #remove the papers with no authors or authors above 50 

cat("Number of papers remaining after filtering publications ]0,50]:", nrow(df_filtered), "\n")

authors_filtered_papers <- unlist(df_filtered$Authors)
authors_filtered_papers <- trimws(authors_filtered_papers)
num_authors_filtered_papers <- length(unique(authors_filtered_papers))
cat("Number of authors in papers with > 0 and <= 50 authors:", num_authors_filtered_papers, "\n")

df_authors <- df_filtered %>%
select(Authors)

```

#build the graph and net files (filtering the authors per publications= include more than x nummber of publication)

```{r}
#here i have filtered the authors to exclude any aithors with less than 2 publications in the datasets
author_pairs <- df_authors %>%
  mutate(pairs = map(Authors, ~ if (length(.x) > 1) combn(.x, 2, simplify = FALSE) else list())) %>%
  unnest(pairs) %>%
  unnest_wider(pairs, names_sep = "_") %>%
  rename(Author1 = pairs_1, Author2 = pairs_2) %>%
  count(Author1, Author2) %>%
  filter(n >= 2) #you can change the filter here

authors_with_2_plus_pubs <- unique(c(author_pairs$Author1, author_pairs$Author2))
cat("Number of authors with >= 2 publications:", length(authors_with_2_plus_pubs), "\n")
...
# Create the graph
institution.graph <- graph_from_data_frame(edges, directed = FALSE)

# Save as Pajek (.net format)
write_graph(institution.graph, file = "author_network.net", format = "pajek")
...
library(igraph)
is_directed(institution.graph)  # Check if the graph is directed
is_weighted(institution.graph) 
...
institution.graph <- simplify(institution.graph, remove.multiple = TRUE, remove.loops = TRUE)

# Convert to network object
institution.net <- asNetwork(institution.graph)
...
is_connected(institution.graph, mode = "weak") # graph
...
cat("Network diameter (average shortest path):", average_shortest_path, "\n")
#The average shortest path length tells you how efficiently information can be spread in the network.


net_diameter <- diameter(institution.graph, directed = FALSE, weights = NA)
...
institution <- read.graph("author_network.net", format="pajek")
# Visualize the network with cutpoints highlighted
# Highlight the cutpoints in a different color
V(institution)$color <- ifelse(V(institution) %in% cutpoints_result, "red", "lightblue") ##the nodes identified as cutpoints are colored red

# Adjust margins
par(mar = c(5, 5, 5, 5))  # Increase margins (bottom, left, top, right)

# Plot the graph
plot(institution, vertex.label = NA, vertex.size = 10, vertex.color = V(institution)$color)
