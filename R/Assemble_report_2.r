...
assemble_report <- function(config, metadata) {
  ...
  years <- get_years_from_to(network$raw, config)
  ...
  for (i in seq_along(years$from)) {
    ...
    current_df <- filter_by_year(network$raw, config$year_column_name, from_year, to_year)
    ...
    # Save network data (before and after filtering)
    ...
    # Update summary statistics dataframe
    author_stats <- get_summary_stats(
      graph = graph,
      network = network
    )
    summary_stats <- rbind(summary_stats, author_stats)
  ...
  rmarkdown::render(
    ...
    output_format = "pdf_document",
    quiet = TRUE,
    params = list(summary_stats = summary_stats)
  )
